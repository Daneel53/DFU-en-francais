...
namespace PFDMainMod
{
    public class PFDMain : MonoBehaviour
    {
        ...
        private void Awake()
        {
            Debug.Log("Begin mod init: Project French Daggerfall Unity");

            // Load the French building mames generator
            ...
            // Load the French grammar module to replace the default grammar module
            GrammarManager.grammarProcessor = (GrammarRules) new FrenchGrammarRules();
            Debug.Log("PFDU: French grammar processor loaded");

            Debug.Log("Finished mod init: Project French Daggerfall Unity");
            mod.IsReady = true;
        }
